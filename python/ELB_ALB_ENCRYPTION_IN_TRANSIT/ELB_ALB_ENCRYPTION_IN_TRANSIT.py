...
Feature:
  In order to: ensure that the traffic is encrypted in transit
           As: a Security Officer
       I want: to verify that the configuration of all listeners use TLS/SSL for all traffic.
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for elb in all_elbv2:
        ...
        if not is_https_listener(alb_all_listeners):
            evaluations.append(build_evaluation(elb['LoadBalancerArn'], 'NON_COMPLIANT', event))
            continue
...
def is_https_listener(listeners):
